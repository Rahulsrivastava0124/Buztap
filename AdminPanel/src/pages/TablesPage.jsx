...
export default function TablesPage() {
  ...
  const selectedTableElapsed = useMemo(() => {
    if (!selectedTableOrder?.createdAt) return "-";
    const createdAtMs = new Date(selectedTableOrder.createdAt).getTime();
              ...
              {selectedTable?.status === "Occupied" ? (
                <div className="mt-3 pt-3 border-t border-border space-y-1.5">
                  ...
                  <p className="text-xs text-muted">
                    ...
                    <span className="font-semibold text-ink">
                      {selectedTableOrder
                        ? `₹${Number(selectedTableOrder.amount || 0).toLocaleString("en-IN")}`
                        : "-"}
                    </span>
                  </p>
                  <p className="text-xs text-muted">
                    Kitchen Response:{" "}
                    <span className="font-semibold text-ink">
                      {kitchenResponseText(selectedTableOrder?.status)}
                    </span>
                  </p>
                  <p className="text-xs text-muted">
                    Order Status:{" "}
